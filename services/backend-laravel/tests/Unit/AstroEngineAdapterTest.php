...
class AstroEngineAdapterTest extends TestCase
{
    public function test_standard_adapter_switch_keeps_contract_and_marks_source(): void
    {
        Config::set('matching.astro_engine_adapter', 'standard');
        ...
        $natalEvidence = (array) data_get($result, 'module_details.natal_chart.evidence', []);
        $pairEvidence = (array) data_get($result, 'module_details.pair_chart.evidence', []);
        $this->assertSame('standard_stub_v1', (string) ($natalEvidence['engine_source'] ?? ''));
